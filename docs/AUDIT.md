...
| Users (per site): members, pending invitations, invite by link, create a client login | Works (the staff row reads "Unknown" only when a `profiles` row is missing, which the mocks never seed) | `screens.spec.ts`; `users.spec.ts`; Deno `clientAccount.test.ts`, `invite-*` functions |
| Agency Team: agency staff only (members of `agency_members`) and their invitations; no client logins | Works | `screens.spec.ts`; Deno `invite-*` functions |
| Agency Clients (`/agency/clients`): every client across the agency's sites with their sites and last login; add (login or invite, several sites at once), reset password (recovery link), resend and cancel an invite, change site access, remove | Works (email delivery is still a stub: invite and reset links are shown to copy) | `clients.spec.ts`; `screens.spec.ts` (staff crawl it; a client is sent to their site); Deno `clientAccess.test.ts` |
| View as client: the site as one client sees it, read-only, with a banner and Exit; survives a reload; never changes the staff member's session or rights | Works (writes are refused in the browser before they leave: `readOnly.test.ts`; the view state: `viewAs.test.ts`) | `clients.spec.ts` "View as client shows the site as the client sees it, saves nothing, and Exit returns to Clients" |
