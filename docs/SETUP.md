...
To confirm: in Supabase press **Edge Functions**. You should see eighteen functions: `github-setup`, `site-connect`, `content-get`, `content-publish`, `content-publish-batch`, `builder-publish`, `site-diagnose`, `site-embed-check`, `invite-create`, `invite-accept`, `client-create`, `client-password-reset`, `password-set`, `form-submit`, `email-test`, `run-scheduled-posts`, `kit-status`, `update-kit` and `finish-setup`. `form-submit` and `run-scheduled-posts` are the only ones that accept callers who are not signed in (`form-submit` from website visitors sending a form; `run-scheduled-posts` from the pg_cron dispatcher, which verifies a Vault token on every call); `supabase/config.toml` turns off their sign-in checks. `stats-ingest` was removed on request in kit 2.9.0 and the deploy workflow deletes it from the project on the next run (safe when it is already gone).
