...
import { dragHeight, dragSpacing, inwardDelta, keyDelta, resizeColumns, resizeImage } from "./handles.ts";
...
describe("spacing handles", () => {
  ...
  it("keeps the other sides, moves the opposite side with Shift, and adds up from the element's own value", () => {
    const current = { left: { value: 2, unit: "rem" as const }, top: { value: 30, unit: "px" as const } };
    expect(dragSpacing(current, measured, "top", 5, { kind: "padding", symmetric: true })).toEqual({ left: { value: 2, unit: "rem" }, top: { value: 35, unit: "px" }, bottom: { value: 35, unit: "px" } });
